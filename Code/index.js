...
$("#smile").on('click', function(e){
    socket.emit('smileee', {value: 0, userId: userId});
});

$("#sensor").on('click', function(e){
    socket.emit('seensor', {value: 0, userId: userId});
});


socket.on('seensor', function(msg) {
    $('#sensor-container span').text(msg.value);
});
...
}html>3 (or built in pin on Intel Galileo Gen2 as well as Intel Edison)
